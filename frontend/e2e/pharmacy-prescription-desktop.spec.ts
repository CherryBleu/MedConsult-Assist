...
test('pharmacy prescription review contains desktop table overflow and detail dialog', async ({ page }) => {
  await page.setViewportSize({ width: 1024, height: 768 })
  await loginViaUI(page, 'staff', 'pharmacy')

  await page.goto('/pharmacy/prescription-review')
  await expect(page.getByRole('heading', { name: '处方审核' })).toBeVisible({ timeout: 15_000 })
  ...
  await page.getByRole('button', { name: /查看 RX202607160002 处方详情/ }).click()
...
test('pharmacy prescription review exposes alert and retry after desktop list failure', async ({ page }) => {
  await page.setViewportSize({ width: 1024, height: 768 })
  await loginViaUI(page, 'staff', 'pharmacy')
  ...
  await expect(page.getByRole('button', { name: /查看 RX202607160002 处方详情/ })).toBeVisible()
  await expectNoHorizontalOverflow(page)
})
