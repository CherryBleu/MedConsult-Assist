...
class MedicalRecordFlowTest {
    ...
    /** 医生本人身份（doctorId=2001，与 createRecord stub 的归属主键一致） */
    ...
    /** 其他医生身份（doctorId=9999，与记录归属不一致 → 越权） */
    private static org.springframework.test.web.servlet.request.MockHttpServletRequestBuilder otherDoctorAuth(
        ...
        return b.header("X-User-Id", "5002")
                .header("X-User-Primary-Role", "DOCTOR")
                .header("X-User-Roles", "DOCTOR")
                .header("X-User-Doctor-Id", "9999");
    }
    /** 药师身份（审方/调剂/完成处方） */
    private static org.springframework.test.web.servlet.request.MockHttpServletRequestBuilder pharmacyAuth(
            org.springframework.test.web.servlet.request.MockHttpServletRequestBuilder b) {
        return b.header("X-User-Id", "7001")
                .header("X-User-Primary-Role", "PHARMACY_ADMIN")
                .header("X-User-Roles", "PHARMACY_ADMIN")
                .header("X-User-Pharmacist-Id", "3001");
    }
    ...
    void stubFeignIdResolution() {
        when(patientFeignClient.resolveId(any()))
                .thenAnswer(inv -> {
                    String patientNo = inv.getArgument(0);
                    if (PATIENT_NO.equals(patientNo)) {
                        return Result.ok(EntityIdDTO.of(1001L));
                    }
                    if ("P_OTHER".equals(patientNo)) {
                        return Result.ok(EntityIdDTO.of(9999L));
                    }
                    return Result.ok(null);
                });
        when(doctorFeignClient.resolveDoctorId(any()))
                .thenAnswer(inv -> {
                    String doctorNo = inv.getArgument(0);
                    if (DOCTOR_NO.equals(doctorNo)) {
                        return Result.ok(EntityIdDTO.of(2001L));
                    }
                    if ("D_OTHER".equals(doctorNo)) {
                        return Result.ok(EntityIdDTO.of(9999L));
                    }
                    return Result.ok(null);
                });
    ...
    void recordDetail_otherDoctorForbidden() throws Exception {
        ...
        // 其他医生（doctorId=9999）查非本人接诊病历 → FORBIDDEN（ASSIGNED 数据范围）
    ...
    void review_notFound() throws Exception {
        mvc.perform(pharmacyAuth(post("/api/v1/prescriptions/RX_NOT_EXIST/review")
                        .contentType("application/json")
    ...
    void review_pendingToApproved() throws Exception {
        ...
        mvc.perform(pharmacyAuth(post("/api/v1/prescriptions/" + rxNo + "/review")
                        .contentType("application/json")
    ...
    void dispense_approvedToDispensed() throws Exception {
        ...
        mvc.perform(pharmacyAuth(post("/api/v1/prescriptions/" + rxNo + "/dispense")
                        .contentType("application/json")
    ...
    void complete_dispensedToCompleted() throws Exception {
        ...
        mvc.perform(pharmacyAuth(post("/api/v1/prescriptions/" + rxNo + "/complete")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(0))
