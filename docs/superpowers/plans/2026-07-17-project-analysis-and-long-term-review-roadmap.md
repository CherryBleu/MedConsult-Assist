...
| 第 3–4 周 | 安全与审计 | RBAC 驱动 scope、入口角色闭环、行级过滤、审计 outbox | 越权测试、scope 测试、审计投递与幂等测试通过 |
| 第 5–6 周 | 业务与性能 | 症状规则、退费、用户列表 O(pageSize)、药品风险批量、调剂异步状态机 | 复杂度和远程调用数下降；故障注入无数据漂移 |
...
| 创建 | `backend/auth-service/src/test/java/com/medconsult/auth/AuthPaginationFlowTest.java` | 验证 auth-service 分页自动配置与 SQL LIMIT |
...
| 修改 | auth、patient、outpatient、drug、medical-record 的关键写 Service | 在同一事务内显式入队审计事件 |
...
| 创建 | `backend/outpatient-service/src/main/java/com/medconsult/outpatient/refund/`、`frontend/src/api/appointment.js` 与 `MyAppointment.vue` 退款入口 | 完成退款单状态机和幂等 |
...
| 修改 | `backend/ai-service/src/main/java/com/medconsult/ai/service/MedicationFunctionService.java` | O(D) 次 Feign 降为 1 次 |
...
- [ ] **步骤 2：先结清 `MedicalRecordFlowTest` WIP 门禁**

读取该文件的 diff，确认改动只包含鉴权身份头、owner/医生越权用例和既有处方请求补角色，不夹带生产代码。运行：
...
若通过，执行 1 轮测试正确性 review 后将该文件独立提交：

```bash
git add backend/medical-record-service/src/test/java/com/medconsult/medicalrecord/MedicalRecordFlowTest.java
git commit -m "test(medical-record): 补齐病历处方鉴权与越权回归"
```

若失败，使用 systematic-debugging 定位测试身份、H2 方言或生产授权根因；不得放宽安全断言、不得丢弃 WIP。只有该测试通过并独立提交后，才进入下一步。

- [ ] **步骤 3：用 worktree 创建实施分支**
...
预期：新 worktree 工作区干净，并包含已经验证的 `MedicalRecordFlowTest` 提交。
...
| # | 状态 | 必须附的证据 |
| --- | --- | --- |
| 1 | 已实现，但统一“8 个状态/9 个总端点/6 个流转端点”口径 | prescription DDL、Controller、状态机测试 |
| 2 | 部分实现 | audit_log 表、Consumer；生产 outbox 缺失 |
| 3 | 部分实现 | RBAC DDL/Entity/Mapper 已有；scope 仍为 `*` |
| 4 | 部分实现 | InternalController/Feign 已有；逐项核对服务鉴权和覆盖 |
| 5 | 部分实现 | RAG 可用；三类规则表仍未接 Mapper |
| 6 | 已实现 | 文本报告/影像分表、Entity、Mapper、消费者 |
| 7 | 部分实现 | sys_service_account 与服务 token 已有；逐服务验证内部鉴权 |
| 8 | 部分实现 | SSE、锁、限流已落地；审计 MQ 生产端缺失 |
| 9 | 部分实现 | 已修字段保留证据；生成未清零项清单 |
| 10 | 已实现，运行依赖需单独验收 | 上传 Controller、MinIO Service、前端调用链 |
| 11 | 部分实现 | 通用状态接口可完成 CHECKED_IN；叫号/复诊仍缺 |
| 12 | 部分实现 | FEFO 已有；退款与检查检验缺失 |
| 13 | 未实现 | `id_no` 明文、无 common-crypto、无统一行级过滤 |
| 14 | 部分实现 | 分页默认值冲突仍在；GET body 示例逐项清理 |
| 15 | 部分实现 | 入口角色与写守卫已修；scope、纯管理员、SELF 过滤仍缺 |
...
def verify_rows(root: Path, rows: list[ContractRow]) -> list[str]:
    ...
    for row in rows:
        ...
        if row.backend != "—":
            ...
            if not contains(source, row.controller_prefix) or not contains(source, row.method_suffix):
                errors.append(f"{row.domain} {row.method} {row.public_path}: backend 映射不完整")
    return errors
```

入口读取 `docs/全栈契约矩阵.md`，有错误时退出码为 1。
...
```bash
git add docs/全栈契约矩阵.md docs/接口文档.md docs/架构设计文档.md docs/数据库设计文档.md scripts/review
...
```bash
git add backend/outpatient-service/pom.xml backend/outpatient-service/src/test
...
Page<TestEntity> page = mapper.selectPage(new Page<>(2, 2),
        new QueryWrapper<TestEntity>().orderByAsc("name"));
assertEquals(5L, page.getTotal());
assertEquals(2, page.getRecords().size());
```

- [ ] **步骤 2：增加 auth-service 自动配置测试**

`AuthPaginationFlowTest` 注入 `ApplicationContext` 与 `SysUserMapper`，断言：
...
git add backend/medconsult-common/common-mybatis/src/test backend/auth-service/src/test backend/auth-service/src/main/java/com/medconsult/auth/user/service/AuthServiceImpl.java
...
无 role 条件时 `selectPage`；有 role 条件时新增 Mapper JOIN 分页查询。禁止逐用户 `resolveRole()`。目标：单请求 SQL 不超过 2 次，Redis 调用不随总用户 U 增长。
...
分别覆盖：患者只能查自己的预约/病历；医生只能查本人接诊或被分配患者；管理员可按授权范围查；直接传其他 userId 不得绕过 token owner。
...
不要引入隐式全局 SQL 魔法。根据 `SecurityContext.requireUser()` 的 patientId/doctorId/role，显式追加 `patient_id = ?`、`doctor_id = ?` 或 appointment 关联条件；请求参数只能进一步收窄，不能放宽 token 范围。

- [ ] **步骤 4：验证 4 类安全用例**

正向、未认证、跨角色、跨 owner 全部通过。检查日志不输出证件号、token 或完整病历内容。

- [ ] **步骤 5：更新架构说明并 Commit**

```bash
git add backend/medical-record-service/src/main backend/medical-record-service/src/test/java/com/medconsult/medicalrecord/MedicalRecordAuthorizationTest.java \
  backend/outpatient-service/src/main backend/outpatient-service/src/test/java/com/medconsult/outpatient/AppointmentAuthorizationTest.java \
  docs/架构设计文档.md
...
- 修改：auth、patient、outpatient、drug、medical-record 服务的 `pom.xml`、`schema.sql`、`application.yml`
- 修改：关键写 Service
...
- [ ] **步骤 2：实现共享事件和 outbox 服务**

```java
@Transactional(propagation = Propagation.MANDATORY)
public void enqueue(AuditLogEvent event) {
...
使用 `MANDATORY` 强制调用者处于业务事务，避免审计与业务分开提交。
...
```bash
git add backend/medconsult-common/common-mq backend/auth-service backend/patient-service backend/outpatient-service \
  backend/drug-service backend/medical-record-service backend/notification-service
...
**文件：**
- 创建：`backend/medconsult-common/common-crypto/`
...
提供 `AesGcmCryptoService.encrypt/decrypt` 与 `Sha256HashService.hashNormalizedIdNo`。密钥只从环境/Nacos 读取，启动时校验 Base64 解码后为 32 bytes。

- [ ] **步骤 3：DDL 增加双写列**

新增 `id_no_ciphertext TEXT`、`id_no_hash CHAR(64)` 和唯一索引 `uk_patient_id_no_hash`。保留旧 `id_no` 作为迁移期回滚列，不再用于新查询。

- [ ] **步骤 4：Service 双写与按 hash 查询**

创建/更新时写 ciphertext + hash；唯一性和精确查询使用 hash；列表 keyword 不再对证件号执行 `LIKE`。响应 DTO 只返回脱敏值。

- [ ] **步骤 5：实现批量迁移器**

按主键游标每批 200 条扫描 `id_no_hash IS NULL AND id_no IS NOT NULL`，在事务内写密文/hash；重复运行只处理未迁移行。迁移由 `medconsult.crypto.migration-enabled=false` 显式开关控制。
...
```bash
git add backend/medconsult-common backend/patient-service
...
- 创建：`HighRiskSymptomRuleEntity.java`、`NegativeRuleEntity.java` 与 3 个 Mapper
...
- 创建：`backend/outpatient-service/src/main/java/com/medconsult/outpatient/refund/` 下 Entity、Mapper、Service、Controller/DTO
...
覆盖 PAID 预约申请退款、重复请求幂等、UNPAID 拒绝、REFUNDED 重复返回原退款单、并发请求只有一张 refund_order。
...
```bash
git add backend/outpatient-service frontend/src/api/appointment.js frontend/src/views/patient/appointment/MyAppointment.vue \
  frontend/e2e/refund.spec.ts docs/接口文档.md docs/数据库设计文档.md
...
- 修改：`backend/drug-service/src/main/java/com/medconsult/drug/service/DrugService.java` 及实现
...
```bash
git add docs/架构设计文档.md backend/medconsult-common/common-feign backend/drug-service backend/ai-service
...
- 修改：medical-record prescription Entity/DTO/Service/Controller/schema
- 创建：common-mq 调剂命令/结果事件
- 创建：drug-service 调剂消费者与幂等记录
- 创建：medical-record-service 调剂结果消费者
...
- [ ] **步骤 8：4 轮 review 与 Commit**

```bash
git add docs backend/medconsult-common/common-mq backend/medical-record-service backend/drug-service \
...
```bash
git add scripts/db/benchmark backend/notification-service/src/main/resources/schema.sql backend/ai-service/src/main/resources/db/schema-ai.sql
...
npm --prefix frontend run build
npm --prefix frontend run test:e2e -- e2e/mobile-responsive.spec.ts
git add frontend/index.html frontend/src/styles frontend/src/composables frontend/src/views/common frontend/e2e/mobile-responsive.spec.ts
...
npm --prefix frontend run build
git add frontend/src/components frontend/src/views frontend/e2e/error-recovery.spec.ts
...
- 修改：`PatientHome.vue`、`DoctorWorkbench.vue`、`PharmacyWorkbench.vue`
- 修改：管理员默认工作台或 `Home.vue`
...
npm --prefix frontend run build
git add frontend/src/styles frontend/src/views frontend/e2e/visual-workbench.spec.ts
...
**文件：**
- 修改：`backend/{gateway,auth-service,patient-service,outpatient-service,drug-service,medical-record-service,notification-service,ai-service}/pom.xml`
- 修改：对应 `src/main/resources/application.yml`
...
```bash
git add backend infra docs/可观测性运行手册.md
...
CI 使用 Playwright bundled Chromium；本地可继续通过 `PW_BROWSER_CHANNEL=chrome` 使用系统 Chrome。
...
- 修改实际使用以下 26 个图标但缺少局部 import 的 Vue 文件：`ArrowDown`、`ArrowLeft`、`Bell`、`Box`、`Calendar`、`CircleCheck`、`Clock`、`Close`、`Cpu`、`Document`、`Expand`、`FirstAidKit`、`Fold`、`InfoFilled`、`Loading`、`Lock`、`Monitor`、`Phone`、`PictureFilled`、`Plus`、`Search`、`Suitcase`、`UploadFilled`、`User`、`UserFilled`、`Warning`
...
若 bundle 报告确认全量 Element Plus 图标占 vendor 主要比例，先移除 `src/main.js:7-16` 的全图标注册。逐文件显式 import 上述 26 个图标；运行 `rg -n '<el-icon|icon:' frontend/src`，确保每个模板/路由引用都能在同文件 import 或统一图标映射中命中。
...
npm --prefix frontend run build
npm --prefix frontend run test:e2e
git add frontend/package.json frontend/package-lock.json frontend/vite.config.mjs frontend/src/main.js frontend/src/views frontend/src/layouts
...
```bash
git add scripts/perf docs/性能基线-2026-07.md
