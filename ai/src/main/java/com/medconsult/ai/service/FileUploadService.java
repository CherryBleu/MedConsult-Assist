...
public class FileUploadService {
    ...
    public ChunkUploadResponse uploadChunk(MultipartFile file, String uploadId, Integer chunkIndex, Integer totalChunks,
        ...
        if (!StringUtils.hasText(filename)) {
            throw new BizException(400001, "filename is required");
        }
        if (chunkIndex == null || totalChunks == null || chunkIndex < 0 || totalChunks <= 0 || chunkIndex >= totalChunks) {
            throw new BizException(400001, "invalid chunk index or total chunks");
        }
        ensureBucket();
    ...
    public FileUploadResponse getFile(String fileId) {
        AiFileUploadEntity entity = fileUploadMapper.selectOne(new LambdaQueryWrapper<AiFileUploadEntity>()
                ...
                .last("limit 1"));
        if (entity == null) {
            throw new BizException(404001, "file not found");
        }
        return toResponse(entity);
    }
    ...
    private void ensureBucket() {
        ...
        try {
            ...
            if (!exists && storage.autoCreateBucket()) {
                ...
                throw new BizException(500001, "MINIO bucket does not exist: " + bucket());
            }
        } catch (BizException ex) {
            throw ex;
        } catch (Exception ex) {
            throw new BizException(500001, "MINIO bucket check failed: " + ex.getMessage());
        }
    }
    ...
    private void putObject(String objectKey, MultipartFile file) {
        try (InputStream in = file.getInputStream()) {
            ...
            throw new BizException(500001, "MINIO upload failed: " + ex.getMessage());
        }
    }
    ...
    private void composeChunks(String uploadId, int totalChunks, String objectKey) {
        try {
            ...
            throw new BizException(500001, "MINIO chunk compose failed: " + ex.getMessage());
        }
    }
    ...
    private long objectSize(String objectKey) {
        try {
            ...
            throw new BizException(500001, "MINIO object stat failed: " + ex.getMessage());
        }
    }
    ...
    private static void validateFile(MultipartFile file) {
        if (file == null || file.isEmpty()) {
            throw new BizException(400001, "file is required");
        }
    }

    private static String required(String value, String message) {
        if (!StringUtils.hasText(value)) {
            throw new BizException(500001, message);
        }
        return value;
    }
